...
const inputNo = document.getElementById("no-of-people").value
...
billbox.addEventListener("keydown",(event)=>{
    ...
    if (event.key === "Enter"){
       ...
       // totalPerHead.value= "$" + (inputBill * inputBill*chosenTip /inputNo).toFixed(2);
        //tipAmountPerhead.innerHTML = "$"+(inputBill * 0.1 /inputNo).toFixed(2); 
...
//to deal with the tip selections

const tipBoxes ={};
//console.log(tipList.tip10)
        ...
        if (element) {
          //  
            element.addEventListener("click",()=>{
                const chosenTip = tipList[key];

                if (lastClickedButton !=null && lastClickedButton  !== element) {
                    lastClickedButton.style.backgroundColor = "hsl(183, 100%, 15%)";  // Reset the previous button color
                }
